...
const query = new Query({host: "142.132.197.195",port: 19132})
module.exports = {
    name: `ready`,
    once: false,
    execute(client){
        ...
        setInterval(() => {
            file_get_contents("https://api.mcsrvstat.us/bedrock/2/142.132.197.195").then(response => {
            
                client.user.setActivity(JSON.parse(response).players["online"] + " player are playing on MagicGames",{
                    type: "LISTENING"
                })
            })
...
client.guilds.cache.get("952994889413591100")?.commands.cache.forEach(command => {
console.log(command.name + "" + command.id)
        ...
        setInterval(() => {
            ...
            axios(api + '/application/nodes/', {
                ...
                else await messages.edit({embeds: [embed]})
